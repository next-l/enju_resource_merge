class SeriesStatementMergesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { render :xml => @series_statement_merges }
    end
  end

  # GET /series_statement_merges/1
  # GET /series_statement_merges/1.xml
  def show
    respond_to do |format|
      format.html # show.html.erb
      format.xml  { render :xml => @series_statement_merge }
    end
  end

  # GET /series_statement_merges/new
  # GET /series_statement_merges/new.xml
  def new
  ...
  # POST /series_statement_merges.xml
  def create
    ...
    respond_to do |format|
      if @series_statement_merge.save
        ...
        format.xml  { render :xml => @series_statement_merge, :status => :created, :location => @series_statement_merge }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @series_statement_merge.errors, :status => :unprocessable_entity }
      end
    end
  end

  # PUT /series_statement_merges/1
  # PUT /series_statement_merges/1.xml
  def update
    respond_to do |format|
  ...
  # DELETE /series_statement_merges/1.xml
  def destroy
